...
- **VLAN:** `opnsense_list_vlans`, `opnsense_add_vlan(iface,vlan,descr,pcid?)`,
  `opnsense_delete_vlan(uuid)` (tag range 1–4094)
- **LAGG:** `opnsense_list_lagg`, `opnsense_add_lagg(ports,descr,mode,primary_member?)`,
  `opnsense_delete_lagg(uuid)`
...
- **access / NTP:** `opnsense_search_users`, `opnsense_set_user_password(username,password_file)`,
  `opnsense_set_timeservers(timeservers,web_user,password_file)` (web-session),
  `opnsense_get_ntp_status`
